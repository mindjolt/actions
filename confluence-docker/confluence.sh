...
function AddPdf() {
  ...
  curl --silent \
    ...
    "$INPUT_URI/$INPUT_PAGE/child/attachment" | jq '. | select(.statusCode != 200) | .message | halt_error(1)'
  [[ $? -ne 0 ]] && exit 1
...
curl \
  ...
  -d @/tmp/body.json --url "$INPUT_URI/$INPUT_PAGE" |  jq '. | select(.statusCode != 200) | .message | halt_error(1)'
if [ $? -ne 0 ]; then
  exit $?
fi
